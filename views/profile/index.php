...
<div class="bg-ios-cardLight dark:bg-ios-cardDark backdrop-blur-3xl shadow-[0_4px_24px_rgba(0,0,0,0.04)] dark:shadow-none active:scale-[0.98] transition-transform duration-200 p-8 rounded-[28px] max-w-3xl relative z-20">
    <form action="<?= base_url('profile/update') ?>" method="POST" enctype="multipart/form-data" class="flex flex-col md:flex-row gap-8">
        ...
        <div class="flex flex-col items-center gap-4 w-full md:w-1/3">
            ...
            <div class="relative group cursor-pointer flex-shrink-0" style="width: 128px; height: 128px;" onclick="document.getElementById('avatar-upload').click()">
                ...
                <?php if(!empty($user['avatar_url'])): ?>
                    <img id="avatar-preview" src="<?= base_url($user['avatar_url']) ?>?v=<?= time() ?>" class="w-full h-full rounded-full border-4 group- transition-colors" style="object-fit: cover; object-position: center; display: block;" onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name=<?= urlencode($user['full_name']) ?>&background=4361ee&color=fff&size=128';">
                    <div id="avatar-preview-fallback" class="w-full h-full rounded-full flex items-center justify-center text-white font-bold text-4xl border-4 group- transition-colors hidden" style="background: linear-gradient(135deg, #4361ee, #7209b7);">
                        <?= strtoupper(substr($user['full_name'] ?? 'U', 0, 1)) ?>
                    </div>
                <?php else: ?>
                    <div id="avatar-preview-fallback" class="w-full h-full rounded-full flex items-center justify-center text-white font-bold text-4xl border-4 group- transition-colors" style="background: linear-gradient(135deg, #4361ee, #7209b7);">
                        <?= strtoupper(substr($user['full_name'] ?? 'U', 0, 1)) ?>
                    </div>
                    <!-- FIX: Empty src triggers base_url() -> loads index.php -> infinite loop. Gunakan Pixel Transparan base64 -->
                    <img id="avatar-preview" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" class="w-full h-full rounded-full border-4 group- transition-colors hidden" style="object-fit: cover; object-position: center;">
                <?php endif; ?>
        ...
        <div class="flex-1 space-y-5">
            <div>
                ...
                <input type="text" name="full_name" value="<?= htmlspecialchars($user['full_name'] ?? '') ?>" class="w-full px-4 py-3 bg-black/30 rounded-2xl text-white focus:ring-2 focus:ring-blue-500/50 outline-none transition-all text-sm" required>
            </div>
            
            <div>
                <label class="block text-xs font-semibold text-white/60 mb-2 uppercase tracking-wide">Email (Login ID)</label>
                <input type="email" value="<?= htmlspecialchars($user['email'] ?? '') ?>" class="w-full px-4 py-3 bg-white/5 rounded-2xl text-white/50 text-sm cursor-not-allowed" readonly disabled title="Email tidak dapat diubah dari sini">
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="phone" value="<?= htmlspecialchars($user['phone'] ?? '') ?>" class="w-full pl-11 pr-4 py-3 bg-black/30 rounded-2xl text-white focus:ring-2 focus:ring-emerald-500/50 outline-none transition-all text-sm placeholder-white/20" placeholder="Contoh: 08123456789">
                </div>
            ...
            <div class="pt-4 border-t">
                ...
                <input type="password" name="password" class="w-full px-4 py-3 bg-black/30 rounded-2xl text-white focus:ring-2 focus:ring-red-500/50 outline-none transition-all text-sm placeholder-white/20" placeholder="Kosongkan jika tidak ingin mengubah password">
            </div>

            <div class="pt-4">
                <button type="submit" class="w-full sm:w-auto px-8 py-3.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl text-sm font-bold shadow-[0_0_15px_rgba(37,99,235,0.4)] hover: hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center">
