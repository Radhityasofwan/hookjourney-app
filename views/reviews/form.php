<div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between relative z-50">
    <div class="flex items-center">
        <a href="<?= base_url('reviews') ?>" class="flex items-center justify-center w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 text-white/70 hover:text-white transition-all mr-4 text-decoration-none">
    ...
    <?php if($review['review_status'] == 'published'): ?>
        <div class="mt-4 sm:mt-0">
            <span class="bg-emerald-500/20 border-emerald-500/30 text-emerald-300 px-4 py-2 text-xs rounded-full font-bold uppercase tracking-wider flex items-center backdrop-blur-md">
...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-40">
    ...
    <div class="lg:col-span-4 space-y-6">
        <div class="bg-ios-cardLight dark:bg-ios-cardDark backdrop-blur-3xl shadow-[0_4px_24px_rgba(0,0,0,0.04)] dark:shadow-none active:scale-[0.98] transition-transform duration-200 p-6 rounded-[28px]">
            ...
            <div class="space-y-4">
                ...
                ?>
                    <div class="flex justify-between items-center bg-black/20 p-4 rounded-[16px] hover:bg-black/30 transition-colors">
            ...
            <div class="mt-8 p-4 rounded-2xl bg-blue-500/10 border-blue-500/20 text-blue-200 text-xs flex items-start">
    ...
    <div class="lg:col-span-8 space-y-6">
        <form action="<?= base_url('reviews/update') ?>" method="POST" class="bg-ios-cardLight dark:bg-ios-cardDark backdrop-blur-3xl shadow-[0_4px_24px_rgba(0,0,0,0.04)] dark:shadow-none active:scale-[0.98] transition-transform duration-200 p-6 rounded-[28px] flex flex-col h-full">
            <input type="hidden" name="review_id" value="<?= $review['id'] ?>">
            
            <h3 class="font-bold text-white/90 mb-6 border-b pb-3 flex items-center text-sm uppercase tracking-wider">
            ...
            <div class="space-y-6 flex-grow">
                ...
                <div class="pt-4 border-t">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <?php if($review['review_status'] !== 'published'): ?>
                        <button type="button" onclick="addActionRow()" class="text-xs bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-2xl transition-colors">
                    ...
                    <div id="action-items-container" class="space-y-3">
                        <?php if(empty($actionItems)): ?>
                            ...
                            <div class="flex items-center gap-3 action-row">
                                <input type="text" name="action_titles[]" placeholder="Deskripsi tugas atau perbaikan..." class="flex-grow px-4 py-2.5 bg-black/30 rounded-xl text-white text-sm outline-none <?= $review['review_status'] == 'published' ? 'opacity-60 cursor-not-allowed' : 'focus:' ?>" <?= $review['review_status'] == 'published' ? 'readonly' : '' ?>>
                                <select name="action_owners[]" class="w-40 px-3 py-2.5 bg-black/30 rounded-xl text-white text-sm outline-none appearance-none <?= $review['review_status'] == 'published' ? 'opacity-60 cursor-not-allowed pointer-events-none' : 'focus:' ?>">
                                    <option value="">PIC (Opsional)</option>
                                    <?php foreach($teamMembers as $tm): ?>
                                        <option value="<?= $tm['id'] ?>"><?= htmlspecialchars($tm['full_name']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                                <input type="date" name="action_dues[]" class="w-36 px-3 py-2.5 bg-black/30 rounded-xl text-white text-sm outline-none [color-scheme:dark] <?= $review['review_status'] == 'published' ? 'opacity-60 cursor-not-allowed' : 'focus:' ?>" <?= $review['review_status'] == 'published' ? 'readonly' : '' ?>>
                                <?php if($review['review_status'] !== 'published'): ?>
                                <button type="button" onclick="this.parentElement.remove()" class="w-10 h-10 flex items-center justify-center bg-red-500/10 text-red-400 rounded-xl border-red-500/20 hover:bg-red-500/20 transition-colors shrink-0">
                        ...
                        <?php else: ?>
                            ...
                            <?php foreach($actionItems as $ai): ?>
                                <div class="flex items-center gap-3 action-row">
                                    <input type="text" name="action_titles[]" value="<?= htmlspecialchars($ai['title']) ?>" placeholder="Deskripsi tugas atau perbaikan..." class="flex-grow px-4 py-2.5 bg-black/30 rounded-xl text-white text-sm outline-none <?= $review['review_status'] == 'published' ? 'opacity-60 cursor-not-allowed' : 'focus:' ?>" <?= $review['review_status'] == 'published' ? 'readonly' : '' ?>>
                                    <select name="action_owners[]" class="w-40 px-3 py-2.5 bg-black/30 rounded-xl text-white text-sm outline-none appearance-none <?= $review['review_status'] == 'published' ? 'opacity-60 cursor-not-allowed pointer-events-none' : 'focus:' ?>">
                                        <option value="">PIC (Opsional)</option>
                                        <?php foreach($teamMembers as $tm): ?>
                                    ...
                                    <input type="date" name="action_dues[]" value="<?= $ai['due_date'] ?>" class="w-36 px-3 py-2.5 bg-black/30 rounded-xl text-white text-sm outline-none [color-scheme:dark] <?= $review['review_status'] == 'published' ? 'opacity-60 cursor-not-allowed' : 'focus:' ?>" <?= $review['review_status'] == 'published' ? 'readonly' : '' ?>>
                                    <?php if($review['review_status'] !== 'published'): ?>
                                    <button type="button" onclick="this.parentElement.remove()" class="w-10 h-10 flex items-center justify-center bg-red-500/10 text-red-400 rounded-xl border-red-500/20 hover:bg-red-500/20 transition-colors shrink-0">
                                        <i class="ph-bold ph-trash"></i>
                                    </button>
            ...
            <?php if($review['review_status'] !== 'published'): ?>
            <div class="pt-6 mt-8 border-t flex flex-col sm:flex-row justify-end gap-4">
                <button type="submit" name="save_draft" class="px-6 py-3 bg-white/10 rounded-full text-white hover:bg-white/20 transition-all font-semibold text-sm">
                    Simpan Draf Review
                </button>
...
function addActionRow() {
    ...
    row.innerHTML = `
        <input type="text" name="action_titles[]" placeholder="Deskripsi tugas atau perbaikan..." class="flex-grow px-4 py-2.5 bg-black/30 rounded-xl text-white text-sm outline-none focus:">
        <select name="action_owners[]" class="w-40 px-3 py-2.5 bg-black/30 rounded-xl text-white text-sm outline-none appearance-none focus:">
            <option value="">PIC (Opsional)</option>
            <?php foreach($teamMembers as $tm): ?>
                <option value="<?= $tm['id'] ?>"><?= htmlspecialchars($tm['full_name']) ?></option>
            <?php endforeach; ?>
        </select>
        <input type="date" name="action_dues[]" class="w-36 px-3 py-2.5 bg-black/30 rounded-xl text-white text-sm outline-none focus: [color-scheme:dark]">
        <button type="button" onclick="this.parentElement.remove()" class="w-10 h-10 flex items-center justify-center bg-red-500/10 text-red-400 rounded-xl border-red-500/20 hover:bg-red-500/20 transition-colors shrink-0">
            <i class="ph-bold ph-trash"></i>
        </button>
    `;
