...
<div class="mb-6 flex flex-col xl:flex-row xl:items-center justify-between gap-4 relative z-50">
    ...
    <div class="flex flex-wrap items-center gap-3">
        ...
        <div class="flex items-center bg-black/40 p-1.5 rounded-[12px] backdrop-blur-md">
            <a href="?v=kanban" class="flex items-center px-3 py-1.5 rounded-[8px] text-xs font-semibold transition-all duration-200 <?= $activeView == 'kanban' ? 'bg-white/10 text-white ' : 'text-white/40 hover:text-white hover:bg-white/5' ?>" title="Kanban Board">
                <i class="ph-fill ph-kanban mr-1.5 text-base"></i> Board
            </a>
            <a href="?v=table" class="flex items-center px-3 py-1.5 rounded-[8px] text-xs font-semibold transition-all duration-200 <?= $activeView == 'table' ? 'bg-white/10 text-white ' : 'text-white/40 hover:text-white hover:bg-white/5' ?>" title="Data Grid">
                <i class="ph-fill ph-table mr-1.5 text-base"></i> Table
            </a>
            <a href="?v=calendar" class="flex items-center px-3 py-1.5 rounded-[8px] text-xs font-semibold transition-all duration-200 <?= $activeView == 'calendar' ? 'bg-white/10 text-white ' : 'text-white/40 hover:text-white hover:bg-white/5' ?>" title="Editorial Calendar">
                <i class="ph-fill ph-calendar-blank mr-1.5 text-base"></i> Calendar
            </a>
            <a href="?v=timeline" class="flex items-center px-3 py-1.5 rounded-[8px] text-xs font-semibold transition-all duration-200 <?= $activeView == 'timeline' ? 'bg-white/10 text-white ' : 'text-white/40 hover:text-white hover:bg-white/5' ?>" title="Gantt Chart">
...
<div class="kanban-scroll flex gap-4 overflow-x-auto pb-6 pt-1 items-start" style="min-height: 70vh; position: relative; z-index: 40;">
    ...
    <div class="kanban-column flex flex-col flex-shrink-0 w-[290px] rounded-[16px] bg-white/5 backdrop-blur-md transition-all duration-300 max-h-[75vh]" 
        ...
        <div class="p-3 border-b flex justify-between items-center bg-black/20 rounded-t-[16px]">
            ...
            <span class="col-counter flex items-center justify-center min-w-[20px] h-[20px] px-1 text-[10px] font-bold text-white/70 bg-white/10 rounded-2xl">
                <?= count($col['items']) ?>
            </span>
        </div>
        
        <div class="column-body flex-1 overflow-y-auto p-3 flex flex-col gap-3 min-h-[100px]">
            <div class="empty-placeholder text-center py-6 rounded-[12px] text-white/30 text-[10px] font-medium flex-col items-center justify-center" style="<?= empty($col['items']) ? 'display: flex;' : 'display: none;' ?>">
            ...
            <?php if (!empty($col['items'])): ?>
                ...
                <div class="content-card p-3 rounded-[12px] relative bg-[#0f0c29]/70 hover:bg-white/10 hover: hover:-translate-y-0.5 hover: transition-all duration-200 group" 
                    ...
                    <div onclick="openContentModal(this)" data-json="<?= htmlspecialchars(json_encode($item), ENT_QUOTES, 'UTF-8') ?>" class="cursor-pointer">
                        ...
                        <div class="flex justify-between items-start mb-2">
                            <div class="flex items-center gap-1.5 flex-wrap">
                                ...
                                <span class="px-1.5 py-0.5 text-[8px] uppercase font-bold rounded bg-white/5 text-white/60 tracking-widest flex items-center">
                            ...
                            <div class="dropdown" onclick="event.stopPropagation()">
                                ...
                                <ul class="dropdown-menu dropdown-menu-dark p-1" style="border-radius: 12px; background: rgba(15,12,41,0.95); backdrop-filter: blur(15px); border: 1px solid rgba(255,255,255,0.15); min-width: 130px; z-index: 100;">
                                    <li>
                                        <a class="dropdown-item py-1.5 px-2.5 rounded-2xl flex items-center text-[11px] text-white/80 hover:text-white hover:bg-white/10" href="<?= base_url('content/edit?id='.$item['id']) ?>">
                                    ...
                                    <li>
                                        <form action="<?= base_url('content/delete') ?>" method="POST" class="m-0" onsubmit="return confirm('Hapus ide konten ini?')">
                                            ...
                                            <button type="submit" class="dropdown-item w-full py-1.5 px-2.5 rounded-2xl flex items-center text-[11px] text-red-400 hover:bg-red-500/20">
                    ...
                    <div class="flex items-center justify-between pt-2.5 border-t">
                        ...
                        <div class="flex items-center gap-1.5">
                            <?php if ($item['pic_user_id']): ?>
                                <?php if (!empty($item['pic_avatar_url'])): ?>
                                    <img src="<?= base_url($item['pic_avatar_url']) ?>" class="w-5 h-5 rounded-full object-cover" title="<?= htmlspecialchars($item['pic_name']) ?>" onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name=<?= urlencode($item['pic_name']) ?>&background=4361ee&color=fff&size=128';">
                                <?php else: ?>
                                ...
                                <?php if(!empty($item['pic_phone'])): ?>
                                    ...
                                    ?>
                                    <a href="https://wa.me/<?= $item['pic_phone'] ?>?text=<?= $waText ?>" target="_blank" onclick="event.stopPropagation()" class="text-emerald-400 bg-emerald-500/10 w-5 h-5 flex items-center justify-center rounded-full hover:bg-emerald-500/20 hover:scale-110 transition-all border-emerald-500/20" title="Ping via WA">
                                        <i class="ph-fill ph-whatsapp-logo text-[11px]"></i>
                                    </a>
                                <?php endif; ?>
                            <?php else: ?>
                                <div class="w-5 h-5 rounded-full flex items-center justify-center bg-white/5 text-white/30" title="Unassigned"><i class="ph-fill ph-user text-[9px]"></i></div>
                            <?php endif; ?>
                        </div>
                        ...
                        ?>
                        <div class="text-[9px] font-medium flex items-center px-1.5 py-0.5 rounded border-transparent <?= $dateClass ?>">
...
<div class="bg-ios-cardLight dark:bg-ios-cardDark backdrop-blur-3xl shadow-[0_4px_24px_rgba(0,0,0,0.04)] dark:shadow-none active:scale-[0.98] transition-transform duration-200 rounded-[20px] relative z-40 overflow-hidden flex flex-col h-[75vh]">
    <div class="px-5 py-4 border-b bg-white/5 flex justify-between items-center">
        ...
        <span class="text-[10px] text-white/50 bg-black/20 px-2.5 py-1 rounded-2xl font-bold"><?= count($rawItems) ?> Ide Terdaftar</span>
    </div>
                    ...
                    <tr class="hover:bg-white/5 transition-colors group cursor-pointer" onclick="openContentModal(this)" data-json="<?= htmlspecialchars(json_encode($item), ENT_QUOTES, 'UTF-8') ?>">
                        ...
                        <td class="px-4 py-3">
                            <?php if($item['scheduled_post_at']): ?>
                                <span class="text-[11px] text-blue-300 font-bold bg-blue-500/10 px-2 py-0.5 rounded border-blue-500/20"><i class="ph-bold ph-calendar-blank mr-1"></i> <?= date('d M Y', strtotime($item['scheduled_post_at'])) ?></span>
                            <?php else: ?>
                        ...
                        <td class="px-4 py-3">
                            <?php if ($item['pic_user_id']): ?>
                                <div class="flex items-center text-[11px] text-white/80 font-medium">
                                    <?php if (!empty($item['pic_avatar_url'])): ?>
                                        <img src="<?= base_url($item['pic_avatar_url']) ?>" class="w-5 h-5 rounded-full object-cover mr-2 shrink-0" onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name=<?= urlencode($item['pic_name']) ?>&background=4361ee&color=fff&size=128';">
                                    <?php else: ?>
...
<div class="bg-ios-cardLight dark:bg-ios-cardDark backdrop-blur-3xl shadow-[0_4px_24px_rgba(0,0,0,0.04)] dark:shadow-none active:scale-[0.98] transition-transform duration-200 rounded-[20px] relative z-40 overflow-hidden flex flex-col min-h-[75vh]">
    ...
    <div class="px-5 py-4 border-b bg-white/5 flex justify-between items-center">
        ...
        <div class="flex items-center gap-2">
            <a href="?v=calendar&month=<?= $prevMonth ?>" class="p-1.5 rounded-2xl bg-white/5 hover:bg-white/10 text-white/70 hover:text-white transition"><i class="ph-bold ph-caret-left text-sm"></i></a>
            <span class="font-bold text-white w-28 text-center tracking-wide text-xs"><?= $currentLabel ?></span>
            <a href="?v=calendar&month=<?= $nextMonth ?>" class="p-1.5 rounded-2xl bg-white/5 hover:bg-white/10 text-white/70 hover:text-white transition"><i class="ph-bold ph-caret-right text-sm"></i></a>
        </div>
    </div>
...
<div class="bg-ios-cardLight dark:bg-ios-cardDark backdrop-blur-3xl shadow-[0_4px_24px_rgba(0,0,0,0.04)] dark:shadow-none active:scale-[0.98] transition-transform duration-200 rounded-[20px] relative z-40 overflow-hidden flex flex-col h-[75vh]">
    ...
    <div class="px-5 py-4 border-b bg-white/5 flex justify-between items-center">
        ...
        <div class="flex items-center gap-2">
            <a href="?v=timeline&month=<?= $prevMonth ?>" class="p-1.5 rounded-2xl bg-white/5 hover:bg-white/10 text-white/70 hover:text-white transition"><i class="ph-bold ph-caret-left text-sm"></i></a>
            <span class="font-bold text-white w-28 text-center tracking-wide text-xs"><?= $currentLabel ?></span>
            <a href="?v=timeline&month=<?= $nextMonth ?>" class="p-1.5 rounded-2xl bg-white/5 hover:bg-white/10 text-white/70 hover:text-white transition"><i class="ph-bold ph-caret-right text-sm"></i></a>
        </div>
    </div>
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <div class="w-56 border-r bg-black/20 z-20 flex flex-col">
            <div class="h-9 border-b px-4 flex items-center text-[9px] font-bold text-white/40 uppercase tracking-widest shrink-0">Nama Konten</div>
            <div class="overflow-y-hidden flex-1 table-scroll" id="tl-left-scroll">
                <?php foreach($timelineItems as $item): ?>
                    <div class="h-10 border-b px-4 flex items-center cursor-pointer hover:bg-white/5 transition" onclick="openContentModal(this)" data-json="<?= htmlspecialchars(json_encode($item), ENT_QUOTES, 'UTF-8') ?>">
...
<div id="briefModal" class="fixed inset-0 z-[1050] hidden items-center justify-center p-4 sm:p-6 opacity-0">
    ...
    <div id="briefContent" class="relative z-10 w-full max-w-4xl bg-[#111126] rounded-2xl shadow-[0_25px_50px_rgba(0,0,0,0.5)] flex flex-col md:flex-row overflow-hidden scale-95 opacity-0 max-h-[90vh]">
        
        <button onclick="closeContentModal()" class="absolute top-4 right-4 z-50 w-8 h-8 rounded-full bg-white/10 backdrop-blur-sm text-white flex items-center justify-center hover:bg-red-500 transition-all">
        ...
        <div class="flex-[1.5] p-6 md:p-8 overflow-y-auto table-scroll bg-black/20" style="background-image: radial-gradient(circle at top left, rgba(67,97,238,0.05) 0%, transparent 50%);">
            ...
            <div class="flex items-center gap-2 mb-3">
                <span id="mdl-status" class="px-2 py-0.5 text-[9px] uppercase font-bold rounded bg-white/10 text-white tracking-widest">STATUS</span>
                <span id="mdl-priority" class="px-2 py-0.5 text-[9px] uppercase font-bold rounded tracking-widest">PRIORITY</span>
            </div>
        ...
        <div class="flex-1 p-6 md:p-8 border-t md:border-t-0 md:border-l bg-[#1a1a35] flex flex-col">
            ...
            <div class="mt-6 pt-5 border-t">
                <a id="mdl-edit-btn" href="#" class="w-full py-2.5 bg-white text-black rounded-2xl text-xs font-bold hover:bg-gray-200 transition-colors flex items-center justify-center">
