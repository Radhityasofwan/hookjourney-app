...
<div class="mb-6 relative z-20 flex flex-col sm:flex-row sm:items-center sm:justify-between">
    ...
    <div class="mt-4 sm:mt-0">
        <a href="<?= base_url('ads/performance') ?>" class="btn px-4 py-2.5 rounded-xl shadow-lg text-sm font-semibold flex items-center transition-all hover:scale-105" 
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-8 relative z-20">
    ...
    <div class="xl:col-span-1">
        <div class="bg-white/5 backdrop-blur-xl rounded-[24px] border border-white/10 shadow-lg p-6 lg:p-8 relative overflow-hidden group">
            ...
            <h3 class="font-bold text-white/90 mb-6 border-b border-white/10 pb-3 flex items-center">
            ...
            <form action="<?= base_url('ads/import/upload') ?>" method="POST" enctype="multipart/form-data" class="space-y-6 relative z-10">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select name="ad_account_id" class="w-full pl-11 pr-4 py-3 bg-black/30 border border-white/10 rounded-2xl text-white focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 outline-none transition-all appearance-none cursor-pointer" required>
                ...
                <div>
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-8 pb-8 border-2 border-white/10 border-dashed rounded-[20px] bg-white/5 hover:bg-white/10 hover:border-blue-400/50 transition-all relative group/dropzone cursor-pointer" onclick="document.getElementById('file-upload').click()">
                        <div class="space-y-2 text-center">
                            ...
                            <div class="flex flex-col text-sm text-white/60 justify-center items-center mt-3">
                                <label for="file-upload" class="relative cursor-pointer rounded-md font-semibold text-blue-400 hover:text-blue-300 focus-within:outline-none transition-colors">
                            ...
                            <div id="file-name-display" class="mt-4 pt-3 border-t border-white/5 text-xs text-white/30 font-medium tracking-wide">
    ...
    <div class="xl:col-span-2">
        <div class="bg-white/5 backdrop-blur-xl rounded-[24px] border border-white/10 shadow-lg flex flex-col h-full overflow-hidden">
            ...
            <div class="px-6 py-5 border-b border-white/10 bg-white/5 flex items-center justify-between">
                ...
                <span class="text-xs font-medium px-2.5 py-1 bg-white/10 rounded-lg text-white/60 border border-white/10">10 Data Terakhir</span>
            </div>
            ...
            <div class="overflow-x-auto scrollbar-hide flex-1">
                <table class="w-full text-left whitespace-nowrap min-w-[700px]">
                    <thead class="bg-black/20">
                        <tr>
                            <th scope="col" class="px-6 py-4 text-xs font-bold text-white/50 uppercase tracking-wider border-b border-white/10">Waktu Diunggah</th>
                            <th scope="col" class="px-6 py-4 text-xs font-bold text-white/50 uppercase tracking-wider border-b border-white/10">Ad Account</th>
                            <th scope="col" class="px-6 py-4 text-xs font-bold text-white/50 uppercase tracking-wider border-b border-white/10">File Report</th>
                            <th scope="col" class="px-6 py-4 text-xs font-bold text-white/50 uppercase tracking-wider border-b border-white/10 text-center">Status</th>
                            <th scope="col" class="px-6 py-4 text-xs font-bold text-white/50 uppercase tracking-wider border-b border-white/10 text-center">Aksi</th>
                        </tr>
                    </thead>
                        ...
                        <?php else: ?>
                            ...
                            <tr class="hover:bg-white/5 transition-colors group">
                                ...
                                <td class="px-6 py-4 text-center align-middle">
                                    <?php 
                                        $badgeClass = 'bg-white/5 border border-white/10 text-white/50'; 
                                        $icon = 'ph-upload-simple';
                                        $label = 'Uploaded';

                                        if ($row['import_status'] == 'processing') {
                                            $badgeClass = 'bg-blue-500/20 border border-blue-500/30 text-blue-300 shadow-[0_0_10px_rgba(59,130,246,0.2)]';
                                            $icon = 'ph-spinner-gap animate-spin';
                                            $label = 'Processing';
                                        } elseif ($row['import_status'] == 'done') {
                                            $badgeClass = 'bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 shadow-[0_0_10px_rgba(52,211,153,0.2)]';
                                    ...
                                    <?php if(!empty($row['error_log']) && $row['import_status'] !== 'uploaded'): ?>
                                        <div class="mt-2 text-[10px] <?= $row['import_status'] == 'failed' ? 'text-red-400 bg-red-900/20 border-red-500/20' : 'text-emerald-400 bg-emerald-900/20 border-emerald-500/20' ?> p-2.5 rounded border text-left leading-relaxed whitespace-normal max-w-[220px] mx-auto">
                                ...
                                <td class="px-6 py-4 text-center align-middle">
                                    <form action="<?= base_url('ads/import/delete') ?>" method="POST" class="m-0" onsubmit="return confirm('Peringatan: Menghapus riwayat ini juga akan menghapus SEMUA Data Performa Iklan yang berasal dari file CSV ini. Yakin ingin melanjutkan? (Anda bisa unggah ulang nantinya)')">
                                        ...
                                        <button type="submit" class="text-red-400 hover:text-red-300 bg-red-500/10 hover:bg-red-500/20 px-2.5 py-2 rounded-lg transition-colors border border-red-500/20 flex items-center justify-center mx-auto" title="Hapus Riwayat & Data Performa">
        ...
        if(e.target.files.length > 0) {
            ...
            fileNameDisplay.innerHTML = `
                <div class="flex flex-col items-center animate-in fade-in zoom-in duration-300">
                    <span class="font-bold text-blue-400 drop-shadow-sm text-sm">${fileName}</span>
