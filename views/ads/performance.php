...
<style>
    ...
    .table-scroll-wrapper::-webkit-scrollbar { height: 10px; width: 10px; }
    .table-scroll-wrapper::-webkit-scrollbar-track { background: rgba(255, 255, 255, 0.02); border-radius: 10px; margin: 0 15px; }
    .table-scroll-wrapper::-webkit-scrollbar-thumb { background: rgba(255, 255, 255, 0.15); border-radius: 10px; border: 2px solid transparent; background-clip: padding-box; }
    .table-scroll-wrapper::-webkit-scrollbar-thumb:hover { background: rgba(255, 255, 255, 0.3); border: 2px solid transparent; background-clip: padding-box; }

    /* Sticky Column Shadow */
    .sticky-col { position: sticky; left: 0; z-index: 20; background: #0a0a1a; }
    .sticky-col::after {
        content: ''; position: absolute; top: 0; right: -15px; width: 15px; height: 100%;
        background: linear-gradient(to right, rgba(0,0,0,0.6), transparent); pointer-events: none;
        opacity: 0; transition: opacity 0.3s;
    }
    .table-scroll-wrapper.is-scrolling .sticky-col::after { opacity: 1; }

    /* Glass Input (Search & Filters) */
    .search-glass, .filter-glass {
        background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.15); color: #ffffff;
        border-radius: 12px; font-size: 0.85rem; transition: all 0.3s ease;
    }
    .search-glass { padding: 0.6rem 1rem 0.6rem 2.5rem; width: 100%; }
    .filter-glass { padding: 0.6rem 1rem; width: 100%; }
    .search-glass::placeholder, .filter-glass::placeholder { color: rgba(255, 255, 255, 0.4); }
    .search-glass:focus, .filter-glass:focus { background: rgba(255, 255, 255, 0.1); border-color: #ff007f; box-shadow: 0 0 0 3px rgba(255, 0, 127, 0.15); outline: none; }
    .filter-glass option { background: #0f0c29 !important; color: #fff !important; }
    
    .search-icon-wrapper { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: rgba(255, 255, 255, 0.4); }
    ...
    .sort-icon { opacity: 0.3; transition: all 0.2s; margin-left: 4px; display: inline-block; }
    .sort-active-asc .sort-icon { opacity: 1; color: #ff007f; transform: rotate(180deg); }
    .sort-active-desc .sort-icon { opacity: 1; color: #ff007f; }
</style>
...
<form method="GET" action="<?= base_url('ads/performance') ?>" class="bg-white/5 backdrop-blur-xl p-5 rounded-[24px] border border-white/10 shadow-lg mb-8 flex flex-col md:flex-row gap-4 md:items-end relative z-20">
    ...
    <div class="flex-1 min-w-[140px]">
        <label class="block text-[11px] font-bold text-white/50 uppercase tracking-wider mb-1.5"><i class="ph ph-calendar-blank mr-1"></i> Dari Tanggal</label>
        <input type="date" name="start_date" value="<?= htmlspecialchars($filters['start_date']) ?>" class="filter-glass cursor-pointer">
    </div>
    <div class="flex-1 min-w-[140px]">
        <label class="block text-[11px] font-bold text-white/50 uppercase tracking-wider mb-1.5"><i class="ph ph-calendar-blank mr-1"></i> Sampai Tanggal</label>
        <input type="date" name="end_date" value="<?= htmlspecialchars($filters['end_date']) ?>" class="filter-glass cursor-pointer">
    </div>
    
    <!-- Tipe Hasil (Result Type) -->
    <div class="flex-[2] min-w-[200px]">
        <label class="block text-[11px] font-bold text-white/50 uppercase tracking-wider mb-1.5"><i class="ph ph-target mr-1"></i> Objektif / Jenis Hasil</label>
    ...
    <div class="bg-white/5 backdrop-blur-xl p-6 rounded-[24px] border border-white/10 shadow-lg relative overflow-hidden group hover:border-white/20 transition-all flex flex-col justify-center">
        ...
        <div>
            <p class="text-xs font-semibold uppercase tracking-wider text-white/50 mb-1 flex items-center justify-between">
                <span class="flex items-center"><i class="ph ph-cursor-click text-pink-400 mr-1.5 text-lg"></i> Avg. CTR</span>
                <?php if($aggCtr == 0 && $summary['total_impressions'] > 0): ?>
                    <i class="ph-fill ph-warning-circle text-pink-400/80 text-lg cursor-help" title="Data Klik Kosong di CSV"></i>
                <?php endif; ?>
            </p>
            ...
            <tbody id="adsTableBody" class="divide-y divide-white/5 bg-transparent">
                ...
                <?php else: ?>
                    ...
                    <?php 
                        ...
                        $statusText = 'Testing';
                        if($camp['ctr'] >= 1.5 && $camp['cpr'] > 0 && $camp['cpr'] <= $aggCpr) $statusText = 'Winner';
                        elseif($camp['spend'] > ($summary['total_spend'] * 0.1) && $camp['results'] == 0) $statusText = 'Fatigue';
                    ?>
