...
<div class="mb-6 flex items-center justify-between relative z-50">
    <div class="flex items-center">
        <a href="<?= base_url('forum') ?>" class="flex items-center justify-center w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 text-white/70 hover:text-white transition-all mr-4 shadow-sm text-decoration-none">
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-2 mt-2">
                <?php if($thread['category_name'] ?? false): ?>
                    <span class="px-2.5 py-0.5 rounded border border-white/10 bg-white/10 text-[9px] font-bold text-white/70 uppercase tracking-widest"><?= htmlspecialchars($thread['category_name']) ?></span>
                <?php endif; ?>
                
                <?php if($thread['is_pinned']): ?>
                    <span class="px-2.5 py-0.5 rounded border border-blue-500/30 bg-blue-500/20 text-[9px] font-bold text-blue-300 uppercase tracking-widest flex items-center"><i class="ph-fill ph-push-pin mr-1.5"></i> Pinned</span>
                <?php endif; ?>
                
                <?php if($thread['is_resolved']): ?>
                    <span class="px-2.5 py-0.5 rounded border border-emerald-500/30 bg-emerald-500/20 text-[9px] font-bold text-emerald-300 uppercase tracking-widest flex items-center"><i class="ph-fill ph-check-circle mr-1.5"></i> Resolved</span>
                <?php endif; ?>
                
                <?php if($thread['is_locked']): ?>
                    <span class="px-2.5 py-0.5 rounded border border-red-500/30 bg-red-500/20 text-[9px] font-bold text-red-300 uppercase tracking-widest flex items-center"><i class="ph-fill ph-lock-key mr-1.5"></i> Locked</span>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    <?php if($user['role_global'] === 'leader'): ?>
        <div class="dropdown">
            <button class="flex items-center justify-center w-10 h-10 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-white transition-all shadow-sm" data-bs-toggle="dropdown">
                <i class="ph-bold ph-dots-three-vertical text-xl"></i>
            </button>
            <ul class="dropdown-menu dropdown-menu-dark shadow-2xl p-1.5" style="border-radius: 16px; background: rgba(15,12,41,0.95); backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.15); min-width: 180px; z-index: 100;">
                <li>
                    <form action="<?= base_url('forum/update-status') ?>" method="POST" class="m-0">
                        ...
                        <button type="submit" class="dropdown-item py-2 px-3 rounded-lg flex items-center text-xs text-white/80 hover:text-white hover:bg-white/10">
                ...
                <li>
                    <form action="<?= base_url('forum/update-status') ?>" method="POST" class="m-0">
                        ...
                        <button type="submit" class="dropdown-item py-2 px-3 rounded-lg flex items-center text-xs <?= $thread['is_resolved'] ? 'text-white/80 hover:text-white' : 'text-emerald-400 hover:text-emerald-300' ?> hover:bg-white/10">
                ...
                <li>
                    <form action="<?= base_url('forum/update-status') ?>" method="POST" class="m-0">
                        ...
                        <button type="submit" class="dropdown-item py-2 px-3 rounded-lg flex items-center text-xs text-orange-400 hover:text-orange-300 hover:bg-white/10">
                ...
                <li>
                    <form action="<?= base_url('forum/delete') ?>" method="POST" class="m-0" onsubmit="return confirm('Hapus seluruh diskusi ini secara permanen?')">
                        <input type="hidden" name="id" value="<?= $thread['id'] ?>">
                        <button type="submit" class="dropdown-item py-2 px-3 rounded-lg flex items-center text-xs text-red-400 hover:bg-red-500/20">
...
<div class="bg-white/5 backdrop-blur-xl rounded-[28px] border border-white/10 shadow-lg overflow-hidden mb-6 relative z-40 flex flex-col">
        ...
        <?php foreach($posts as $idx => $p): ?>
            <div class="p-6 <?= $idx === 0 ? 'bg-white/5' : '' ?> transition-colors hover:bg-white/5">
                <div class="flex items-start">
                    ...
                    <div class="flex-shrink-0 h-10 w-10 md:h-12 md:w-12 rounded-full border border-white/20 shadow-md overflow-hidden bg-black/50">
                    ...
                    <div class="ml-4 flex-1">
                        <div class="flex items-baseline justify-between mb-2">
                            <div class="flex items-center">
                                ...
                                <?php 
                                    $roleColor = 'bg-white/10 text-white/70 border-white/10';
                                    if ($p['role_global'] == 'leader') $roleColor = 'bg-purple-500/20 text-purple-300 border-purple-500/30';
                                ?>
                                <span class="text-[9px] font-bold uppercase <?= $roleColor ?> border backdrop-blur-md px-2 py-0.5 rounded-full ml-3 tracking-wider"><?= $p['role_global'] ?></span>
                            </div>
    ...
    <?php if(!$thread['is_locked']): ?>
    <div class="p-5 bg-black/20 border-t border-white/10 backdrop-blur-2xl">
        <form action="<?= base_url('forum/reply') ?>" method="POST" class="flex flex-col sm:flex-row gap-4 items-end sm:items-center">
            ...
            <div class="flex-grow w-full relative">
                <textarea name="post_text" rows="1" class="w-full px-5 py-3.5 bg-white/5 border border-white/10 rounded-3xl text-white focus:ring-2 focus:ring-white/20 focus:border-white/30 outline-none transition-all text-sm placeholder-white/40 resize-none min-h-[52px]" placeholder="Ketik balasan Anda di sini..." required style="border-radius: 26px;"></textarea>
            </div>
