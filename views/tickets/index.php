<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between relative z-50">
    <div>
        ...
        <p class="text-white/60 text-sm mt-1">Kelola penugasan tim untuk Brand <strong class="text-white"><?= htmlspecialchars($activeBrand['name']) ?></strong></p>
    </div>
    
    <?php if($user['role_global'] === 'leader'): ?>
    <a href="<?= base_url('tickets/create') ?>" class="mt-4 sm:mt-0 flex items-center justify-center px-5 py-2.5 bg-white text-black rounded-full font-semibold shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:scale-105 active:scale-95 transition-all text-sm w-max text-decoration-none">
    ...
    <div class="overflow-x-auto scrollbar-hide flex-grow">
        <table class="min-w-full divide-y divide-white/5">
            <thead class="bg-transparent">
                <tr>
                    <th class="px-6 py-4 text-left text-[10px] font-bold text-white/50 uppercase tracking-widest w-16">ID</th>
                    <th class="px-6 py-4 text-left text-[10px] font-bold text-white/50 uppercase tracking-widest">Tugas / Tiket</th>
                    <th class="px-6 py-4 text-left text-[10px] font-bold text-white/50 uppercase tracking-widest">Status</th>
                    <th class="px-6 py-4 text-left text-[10px] font-bold text-white/50 uppercase tracking-widest">Prioritas</th>
                    <th class="px-6 py-4 text-left text-[10px] font-bold text-white/50 uppercase tracking-widest">PIC (Assignee)</th>
                    <th class="px-6 py-4 text-left text-[10px] font-bold text-white/50 uppercase tracking-widest">Due Date</th>
                    <?php if($user['role_global'] === 'leader'): ?>
                        <th class="px-6 py-4 text-center text-[10px] font-bold text-white/50 uppercase tracking-widest w-24">Aksi</th>
                    <?php endif; ?>
                </tr>
            </thead>
            <tbody class="divide-y divide-white/5 bg-transparent">
                ...
                <?php else: ?>
                    ...
                    <tr class="hover:bg-white/5 cursor-pointer transition-colors group" onclick="openTicketModal(<?= $t['id'] ?>)">
                        
                        <td class="px-6 py-4 whitespace-nowrap text-xs font-bold text-white/40 group-hover:text-blue-400 transition-colors">
                            #<?= htmlspecialchars(explode('-', $t['ticket_no'])[2] ?? $t['ticket_no']) ?>
                        </td>
                        
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2 mb-1.5">
                                <p class="text-sm font-semibold text-white/90 leading-tight group-hover:text-white transition-colors truncate max-w-[280px]"><?= htmlspecialchars($t['title']) ?></p>
                                <!-- Indikator Ekstra -->
                                <?php if($t['comment_count'] > 0): ?>
                                    <span class="text-[10px] text-white/40 flex items-center"><i class="ph-fill ph-chats mr-1 text-blue-400"></i> <?= $t['comment_count'] ?></span>
                                <?php endif; ?>
                                <?php if($t['checklist_total'] > 0): ?>
                                    <span class="text-[10px] text-white/40 flex items-center <?= $t['checklist_done'] == $t['checklist_total'] ? 'text-emerald-400' : '' ?>"><i class="ph-bold ph-check-square-offset mr-1"></i> <?= $t['checklist_done'] ?>/<?= $t['checklist_total'] ?></span>
                                <?php endif; ?>
                            </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap" onclick="event.stopPropagation()">
                            ...
                            <div class="relative inline-flex items-center">
                                <select onchange="updateTicketStatus(<?= $t['id'] ?>, this.value)" class="appearance-none bg-transparent border text-[10px] font-bold uppercase tracking-wider py-1 pl-3 pr-7 rounded-full cursor-pointer focus:outline-none <?= $sColor ?> backdrop-blur-md transition-all shadow-sm">
                                    ...
                                    <option value="cancelled" class="bg-gray-900 text-white" <?= $t['ticket_status'] == 'cancelled' ? 'selected' : '' ?>>CANCEL</option>
                                </select>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <?php
                                $pColor = 'text-white/50'; $pIcon = 'ph-minus';
                                if($t['priority'] == 'high') { $pColor = 'text-orange-400'; $pIcon = 'ph-arrow-up'; }
                                if($t['priority'] == 'urgent') { $pColor = 'text-red-400 drop-shadow-[0_0_8px_rgba(248,113,113,0.5)]'; $pIcon = 'ph-warning-circle'; }
                            ?>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap" onclick="event.stopPropagation()">
                            <?php if($t['assignee_name']): ?>
                                <div class="flex items-center">
                                    ...
                                    <span class="text-sm text-white/80 font-medium truncate max-w-[120px]"><?= htmlspecialchars($t['assignee_name']) ?></span>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <?php if($t['due_at']): ?>
                                ...
                                <span class="flex items-center <?= $isOverdue ? 'text-red-400 font-bold' : 'text-white/60 font-medium' ?>">
                                    <i class="ph-bold ph-calendar-blank mr-1.5"></i> <?= date('d M Y', strtotime($t['due_at'])) ?>
                                </span>
                            <?php else: ?>
...
<style>
    /* Transisi Modal */
    #ticketModal { transition: opacity 0.2s ease; }
    #ticketContent { transition: all 0.2s cubic-bezier(0.16, 1, 0.3, 1); }
</style>
...
<div id="ticketModal" class="fixed inset-0 z-[9999] hidden items-center justify-center p-4 sm:p-6 opacity-0">
    ...
    <div id="ticketContent" class="relative z-10 w-full max-w-5xl bg-[#111126] border border-white/10 rounded-3xl shadow-[0_25px_50px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden scale-95 opacity-0 max-h-[90vh]">
        ...
        <div id="mdl-loader" class="flex-1 flex flex-col items-center justify-center p-20 min-h-[400px]">
            ...
            <p class="text-white/50 text-sm font-medium tracking-wider uppercase animate-pulse">Menarik Data Tiket...</p>
        </div>
        ...
        <div id="mdl-content-body" class="hidden flex-1 overflow-hidden flex-col md:flex-row">
            ...
            <div class="flex-1 p-6 md:p-8 border-t md:border-t-0 md:border-l border-white/10 bg-[#1a1a35] flex flex-col overflow-y-auto table-scroll">
                ...
                <div class="space-y-6 flex-1">
                    ...
                    <div>
                        <p class="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-2">Dibuat Oleh / Tanggal</p>
...
<!-- ==============================================================
     ...
     ============================================================== -->
<script>
    // 1. UPDATE STATUS LANGSUNG DARI TABEL
