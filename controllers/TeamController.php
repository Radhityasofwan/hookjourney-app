    ...
    public function store()
    {
        ...
        try {
            ...
            if ($existingUser) {
                ...
                // Update nama dan phone (dan password jika diisi).
                // FIX MASALAH SINKRON: JANGAN mengubah Role Global user jika dia sudah ada. 
                // Role Global mengontrol workspace utama mereka, biarkan Role In Brand yang mengatur akses spesifik brand ini!
                $userUpdates = ["full_name = ?", "phone = ?", "is_active = 1"];
                ...
                $updateParams[] = $userId;
                $updateSql = "UPDATE users SET " . implode(", ", $userUpdates) . " WHERE id = ?";
            ...
            $db->commit();
            $_SESSION['team_success'] = "Selesai! Informasi anggota '{$fullName}' dan hak aksesnya di brand ini telah tersinkronisasi sempurna.";
        } catch (Exception $e) {
            if ($db->inTransaction())
                $db->rollBack();
